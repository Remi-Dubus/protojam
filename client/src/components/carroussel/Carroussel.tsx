...
function Carroussel({ characters }: { characters: charactersType[] }) {
  ...
  const alimentation = characters.filter((alim) => alim.Catégorie.includes("Alimentation"))
  const biens = characters.filter((bien) => bien.Catégorie.includes("Biens"))
  const serv = characters.filter((service) => service.Catégorie.includes("Services"))
  const equip = characters.filter((equipement) => equipement.Catégorie.includes("Equipements"))
          ...
          {alimentation.map((c) => (
            <figure className={styles.cards} key={c.ID}>
              ...
              <figcaption id={`${c.ID}`}>
                ...
                <h3><em>{c.Catégorie}</em></h3>
                <h3><em>{c.Description}</em> </h3>
                <h3><em>{c.Price_level}</em> </h3>
              </figcaption>
            </figure>
          ))}
        </Slider>
